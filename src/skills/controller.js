const debug = require('debug')('app:module-skills-controller');
...
module.exports.SkillsController = {
    ...
    createSkill: async (req, res, next) => {
        try {
            const { body } = req;
            const insertedId = await SkillsService.create(body);

            Response.success(res, 201, 'Skill creada correctamente', {
                id: insertedId
            });
        } catch (error) {
            next(error);
    ...
    updateSkill: async (req, res, next) => {
        try {
            ...
            const result = await SkillsService.updateSkill(id, body);
